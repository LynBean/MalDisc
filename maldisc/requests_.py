...
async def to_json(response):
    if response.status in (200, 304):
        ...
        raise RequestsBadReqeusts(response.message)
    
    elif response.status == 404:
        raise RequestsNotFound(response.message)
    
    elif response.status == 429:
        raise RequestsTooManyRequests(response.message)
    
    elif response.status == 500:
        raise RequestsInternalServerError(response.message)
    
    elif response.status == 503:
        raise RequestsServiceUnavailable(response.message)
    
    else:
        raise


async def JikanRequests(url: str) -> dict:
